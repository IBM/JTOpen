		...
		XMLErrorHandler xh = new XMLErrorHandler(m_docName, 0);
            ...
            SAXParser parser = new SAXParser();
             try {
               ...
               parser.setFeature("http://apache.org/xml/features/continue-after-fatal-error", false); // TBD: This is to eliminate hang condition if rfml.dtd is not found by SAXParser.parse().  See below.
               parser.setFeature( "http://xml.org/sax/features/namespaces", false );
             }
             catch (org.xml.sax.SAXException se) {
               if (Trace.isTraceErrorOn()) se.printStackTrace(Trace.getPrintWriter());
                ///System.out.println("Exception = " + se);
             }
             parser.setErrorHandler(xh);
             parser.setContentHandler(this);
             parser.setEntityResolver(this);  // So that we can find the rfml.dtd for the parser.

            // Create an InputSource for passing to the parser.
            // Wrap any SAXExceptions as ParseExceptions.
            try
            {
                parser.parse(new InputSource(inStream)); // TBD: This hangs if rfml.dtd can't be found and "continue-after-fatal-error" is set to true.
             }
