...
///           // Note: java.util.Arrays is new in Java2.
    ...
    /**
     Determines the size of buffer (number of bytes) that is required to hold all of the data for this node and its children.
     **/
    ...
    int parseBytes(byte[] bytes)
           throws XmlException
    {
      // Set up the buffer for the bytes.
      int requiredBufferSize = getRequiredLength(this, getRootNode());

      // Check that the input buffer has the right length.
      if (requiredBufferSize > bytes.length)
      {
      ...
      else if (requiredBufferSize < bytes.length)
      {
        throw new XmlException(DAMRI.EXCESS_INPUT_DATA, new Object[] {Integer.toString(requiredBufferSize), Integer.toString(bytes.length), "<recordformat>", this.getNameForException()} );
      }
           ...
           throws XmlException
    {
      ...
      byte[] bytes = new byte[getRequiredLength(this, getRootNode())];
