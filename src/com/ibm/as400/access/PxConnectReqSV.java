...
/**
The PxConnectReqSV class represents the
server view of a connect request.
**/
...
@param secure                   true if this is for a secure connection,
                                false otherwise.
**/
    public PxConnectReqSV (Vector threadGroup,
...
**/
    public PxRepSV process ()
    {
        ...
        if (peer == null) {
            ...
            PSConnection connection = new PSConnection (controller_.getConnectionId (),
                                                                          controller_.getConnectedSocket (),
                                                                          controller_.getInputStream (), 
                                                                          controller_.getOutputStream (),
                                                                          load_);
            connection.start();
            threadGroup_.addElement (connection);

            Verbose.println (ResourceBundleLoader.substitute (PROXY_CONNECTION_ACCEPTED_, 
                                                              new Object[] { controller_, 
                                                                             controller_.getClientAddress (),
                                                                             Long.toString (controller_.getConnectionId ()) }));
        }

        // If rejected, then do nothing.
        else {
            reply = new PxRejectRepSV (peer, secure_);
            if (peer.length () == 0) 
                Verbose.println (ResourceBundleLoader.substitute (PROXY_CONNECTION_REJECTED_, 
                                                                  controller_,
                                                                  controller_.getClientAddress ()));
            else 
                Verbose.println (ResourceBundleLoader.substitute (PROXY_CONNECTION_REDIRECTED_,
                                                                  new Object[] { controller_,
                                                                                 controller_.getClientAddress (),
                                                                                 peer }));
        }
