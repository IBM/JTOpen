...
class ConvTable1208 extends ConvTable
{
  ...
  **/
  final String byteArrayToString(byte[] buf, int offset, int length, int type)
  {
    ...
    if (Trace.traceOn_)
    {
      Trace.log(Trace.CONVERSION, "Destination string for ccsid: " + ccsid_, ConvTable.dumpCharArray(out));
    }
  ...
  **/
  final byte[] stringToByteArray(String source, int type)
  {
    if (Trace.traceOn_)
    {
      Trace.log(Trace.CONVERSION, "Converting string to byte array for ccsid: " + ccsid_, ConvTable.dumpCharArray(source.toCharArray(), 0, source.length()));
    }
    ...
    for (int i=0; i<len; ++i)
    {
      ...
      if (c > 0xD7FF && c < 0xDC00)
      {
        ...
        else
        {
          ...
          if (Trace.traceOn_)
          {
            Trace.log(Trace.CONVERSION, "Fault-tolerant in mid-surrogate. Destination byte array for ccsid: " + ccsid_, ret);
          }
          return ret;
        }
      }
    ...
    if (Trace.traceOn_)
    {
      Trace.log(Trace.CONVERSION, "Destination byte array for ccsid: " + ccsid_, ret);
    }
    return ret;
  }
  ...
  final byte[] stringToByteArray(char[] src, int offset, int length)
  {
    if (Trace.traceOn_)
    {
      Trace.log(Trace.CONVERSION, "Converting string to byte array for ccsid: " + ccsid_, ConvTable.dumpCharArray(src, offset, length));
    }
  ...
  final void stringToByteArray(String source, byte[] buf, int offset) throws CharConversionException
  {
    ...
    if (Trace.traceOn_)
    {
      Trace.log(Trace.CONVERSION, "Converting string to byte array for ccsid: " + ccsid_, ConvTable.dumpCharArray(source.toCharArray()));
    }
    try
    {
      int len = source.length();
      for (int i=0; i<len; ++i)
      {
        ...
        if (c > 0xD7FF && c < 0xDC00)
        {
          ...
          else
          {
            ...
            if (Trace.traceOn_)
            {
              Trace.log(Trace.CONVERSION, "Fault-tolerant in mid-surrogate. Destination byte array for ccsid: " + ccsid_, buf, offset, bufCount-offset);
            }
            return;
          }
        }
    ...
    if (Trace.traceOn_)
    {
      Trace.log(Trace.CONVERSION, "Destination byte array for ccsid: " + ccsid_, buf, offset, bufCount-offset);
    }
  }
      ...
      for (int i=0; i<len && bufCount < max; ++i)
      {
        ...
        if (c > 0xD7FF && c < 0xDC00)
        {
          ...
          else
          {
            ...
            if (Trace.traceOn_)
            {
              Trace.log(Trace.CONVERSION, "Fault-tolerant in mid-surrogate. Destination byte array for ccsid: " + ccsid_, buf, offset, length);
            }
            return;
          }
        }
  ...
  {
    stringToByteArray(source, buf, offset, length); // Don't have a Bidi string type for UTF-8.
  }
}