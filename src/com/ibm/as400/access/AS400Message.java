...
public class AS400Message implements Serializable
{
    ...
    public String getText()
    {
        ...
        // See if we need to insert the substitution text.
        ...
        // In such cases, the message might have only substitution data.
            ...
            (text_ == null || text_.trim().length() == 0))
        {
          try
          {
            Trace.log(Trace.DIAGNOSTIC, "Performing implicit load of additional message data.");
            load();
          }
          catch (Exception e) {
    ...
    public void load(int helpTextFormatting) throws AS400SecurityException, ErrorCompletingRequestException, IOException, InterruptedException, ObjectDoesNotExistException
    {
        ...
        if (libraryName_ == null || fileName_ == null || libraryName_.trim().length() == 0 || fileName_.trim().length() == 0)
        {
          ...
          // If text is blank but we have substitution data, just copy the substitution data into text.
          if ((substitutionData_ != null && substitutionData_.length != 0) &&
              (text_ == null || text_.trim().length() == 0))
          {
            int ccsid;
            ...
            Trace.log(Trace.DIAGNOSTIC, "Message text is blank. Copying substitution data to message text.");
