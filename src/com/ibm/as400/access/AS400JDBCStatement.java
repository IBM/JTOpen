...
implements Statement
{
    ...
    throws SQLException
    {
        ...
        if ((! allowImmediate_) || (! sqlStatement.isImmediatelyExecutable ()))
        {
            ...
            try
            {
                ...
                try
                { 
                    int requestedORS = DBSQLRequestDS.ORS_BITMAP_RETURN_DATA+DBSQLRequestDS.ORS_BITMAP_SQLCA;      //@F3M
                    //@F3A If we are on a server that supports extended column descriptors and if the              //@F3A
                    //@F3A user asked for them, send the extended column descriptors code point.                   //@F3A
                    boolean extendedMetaData = false;                                                              //@F3A
                    if (connection_.getVRM() >= JDUtilities.vrm520)                                                //@F3A
                    {                                                                                              //@F3A
                        extendedMetaData = connection_.getProperties().getBoolean(JDProperties.EXTENDED_METADATA); //@F3A
                        if (extendedMetaData)                                                                      //@F3A
                        {                                                                                          //@F3A
                            requestedORS = requestedORS + DBSQLRequestDS.ORS_BITMAP_EXTENDED_COLUMN_DESCRIPTORS;   //@F3A
                        }                                                                                          //@F3A
                    }                                                                                              //@F3A

                    //@P0A
                    request = DBDSPool.getDBSQLRequestDS(functionId, id_, requestedORS, 0); //@P0C @F3C
                    ...
                    //@F3A If user asked us to parse out extended metadata, then make an object
                    if (extendedMetaData)                                                         //@F3A
                    {                                                                             //@F3A
                        extendedColumnDescriptors_ = reply.getExtendedColumnDescriptors();        //@F3A
                    }                                                                             //@F3A
        ...
        if (nameOverride_.length() == 0)
        {
            ...
            {  // @E7C
                ...
                try
                {
                    ...
                    request = DBDSPool.getDBSQLRequestDS (DBSQLRequestDS.FUNCTIONID_EXECUTE_IMMEDIATE, id_, DBSQLRequestDS.ORS_BITMAP_RETURN_DATA+DBSQLRequestDS.ORS_BITMAP_SQLCA, 0); //@P0C
            ...
            else
            {
                ...
                try
                {
                    ...
                    request = DBDSPool.getDBSQLRequestDS(DBSQLRequestDS.FUNCTIONID_PREPARE_DESCRIBE, id_, DBSQLRequestDS.ORS_BITMAP_RETURN_DATA+DBSQLRequestDS.ORS_BITMAP_DATA_FORMAT, 0); //@P0C
  ...
  **/
    ...
    **/
    ...
    ResultSet object is returned.  Currently the server supports 
    returning only one auto-generated key -- the key for the last inserted row.
    ...
    @exception SQLException     If the statement is not open,
                                if this method is called before a statement
                                has been executed, if auto-generated keys
                                were not requested during this Statement object's creation, 
                                or an error occurs.
    @since Modification 5
    **/
    ...
    throws SQLException
    {
        synchronized(internalLock_)
        {
            ...
            //Result Set is currently open.
            checkOpen ();
            //If we have not made any generated keys yet, a statement must
            //not have been executed, so throw a function sequence error.
    ...
    **/
    ...
    @return     The cursor holdability.  Valid values are ResultSet.HOLD_CURSORS_OVER_COMMIT and 
                ...
                Full functionality of #1 and #2 requires support in the release of OS/400 
    ...
    **/
    ...
    throws SQLException
    {
        ...
        if (returnCode < 0)
        {
            ...
            // just make an empty result set to return to the user.
            generatedKey = "";
            if (JDTrace.isTraceOn())
                JDTrace.logInformation (this, "return code was not 0, generated key is: ''");
        }
        else
        {
            ...
            // key for us.
            generatedKey = sqlca.getGeneratedKey(connection_.converter_); //@P0C
            if (JDTrace.isTraceOn())
                JDTrace.logInformation (this, "generated key from server is: " + generatedKey);
        }   
        Object[][] data = {{generatedKey}};
        String[] fieldNames = {""};
        SQLData[] sqlData   = { new SQLVarchar (32, settings_)};
    ...
    throws SQLException
    {
        if (rpbSyncNeeded_)
        {
            ...
            try
            {
                ...
                // Check server level before sending new code point
                if (connection_.getVRM() >= JDUtilities.vrm520)                                    // @F4A
                {   
                    if (resultSetHoldability_ == AS400JDBCResultSet.CLOSE_CURSORS_AT_COMMIT)       // @F4A
                        request.setResultSetHoldabilityOption((byte)0xD5);                         // @F4A
                    else if (resultSetHoldability_ == AS400JDBCResultSet.HOLD_CURSORS_OVER_COMMIT) // @F4A 
                        request.setResultSetHoldabilityOption((byte)0xE8);                         // @F4A           
                }
