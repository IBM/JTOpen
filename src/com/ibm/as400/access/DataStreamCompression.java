                           ...
                           byte escape)
   {
       ...
       while (i < sourceLength && overflow != true) {
          ...
          // Compare two bytes with next two bytes.
             ...
             int saveOffset = i;
             int repeater = BinaryConverter.byteArrayToUnsignedShort(source, i); // @A2C
             int count = 1;                                                      // @A2C
             i += 2;
             // Calculate the number of times these two bytes are repeated.
             while (((i+1) < sourceLength) &&
                    (BinaryConverter.byteArrayToUnsignedShort(source, i) == repeater)) { // @A2C
                count++;
                i += 2;
             }
             ...
             if (repeatLength >= REPEATER_RECORD_SIZE * 2) {
                ...
                if (j + REPEATER_RECORD_SIZE <= destinationLength) {
                   ...
                   destination[j++] = escape;
                   BinaryConverter.unsignedShortToByteArray(repeater, destination, j); // @A2C
                   BinaryConverter.unsignedShortToByteArray(count, destination, j+2); // @A2C
                   j += 4;
                }
                // Destination array overflow
                else {
       ...
       if (!overflow && (returnCount < length)) {                           // @A1C
          if (traceDiagnostic)
             ...
             if ((i + ESCAPE_SIZE) < sourceLength) {
                ...
                // have an escape byte followed by a non-escape byte
                   ...
                   if ((i + REPEATER_SIZE + COUNT_SIZE) < sourceLength) {
                      ...
                      // Get repeater
                      int repeater = BinaryConverter.byteArrayToUnsignedShort(source, i + ESCAPE_SIZE); // @A2C
                      // Get repeat count
                      int count = BinaryConverter.byteArrayToUnsignedShort(source, i + ESCAPE_SIZE + REPEATER_SIZE); // @A2C

                      // Bytes fit in destination array
                      if ((j + (count * REPEATER_SIZE)) <= destinationLength) {
                         // Write out the bytes to destination array  
                         for (int k = 1; k <= count; ++k) { // @A2C
                            BinaryConverter.unsignedShortToByteArray(repeater, destination, j); // @A2C
                            j += REPEATER_SIZE;
                         }
                      }
                ...
                if ((i + ESCAPE_SIZE) < sourceLength) {
                   ...
                   // have an escape byte followed by a non-escape byte
                      ...
                      if ((i + REPEATER_SIZE + COUNT_SIZE) < sourceLength) {
                         ...
                         // Write out the bytes to destination array  
                         for (int k = 1; k <= count; k++) { // @A2C
                            BinaryConverter.unsignedShortToByteArray(repeater, returnBytes, j); // @A2C
                            j += REPEATER_SIZE;
                         }
                         i += REPEATER_RECORD_SIZE;
                      }
