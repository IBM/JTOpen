...
public class KeyedDataQueue extends BaseDataQueue
{
  private static final String copyright = "Copyright (C) 1997-2000 International Business Machines Corporation and others.";
    ...
    {
        super();
        if (Trace.isTraceOn()) Trace.log(Trace.DIAGNOSTIC, "Constructing KeyedDataQueue object.");
    }
    ...
    {
        if (Trace.isTraceOn()) Trace.log(Trace.DIAGNOSTIC, "Clearing keyed data queue.");
    ...
    {
        if (Trace.isTraceOn()) Trace.log(Trace.DIAGNOSTIC, "Creating keyed data queue.");
    ...
    {
        if (Trace.isTraceOn()) Trace.log(Trace.DIAGNOSTIC, "Getting key length.");
        open();
        if (Trace.isTraceOn()) Trace.log(Trace.DIAGNOSTIC, "Key length:", keyLength_);
        return keyLength_;
    }
    ...
    {
        if (Trace.isTraceOn()) Trace.log(Trace.DIAGNOSTIC, "Peeking keyed data queue.");
    ...
    public KeyedDataQueueEntry read(byte[] key, int wait, String searchType) throws AS400SecurityException, ErrorCompletingRequestException, IOException, IllegalObjectTypeException, InterruptedException, ObjectDoesNotExistException
    {
        ...
        String search = searchType.toUpperCase();
        if (!(search.equals("EQ") || search.equals("NE") || search.equals("LT") ||
              search.equals("LE") || search.equals("GT") || search.equals("GE") ))
        {
            Trace.log(Trace.ERROR, "Value of parameter 'searchType' is not valid: " + searchType);
            throw new ExtendedIllegalArgumentException("searchType (" + searchType + ")", ExtendedIllegalArgumentException.PARAMETER_VALUE_NOT_VALID);
        }
    ...
    {
        if (Trace.isTraceOn()) Trace.log(Trace.DIAGNOSTIC, "Retrieving keyed data queue attributes.");
    ...
    {
        if (Trace.isTraceOn()) Trace.log(Trace.DIAGNOSTIC, "Writing keyed data queue.");
