...
public final class IdentityToken implements Serializable, Destroyable {
  ...
  /**
   Parses the byte-array argument into a new IdentityToken object.
   **/
  public static IdentityToken parse(byte[] tokenBytes)  // TBD: Leave this public for testing; make it private before shipping???
    throws IOException, ParseException, EimException
  {
    ...
    try
    {
      ...
      if (counter == 1) {  // This IdentityToken has a single manifest.
        // Parse the User Token.
        UserToken userToken = UserToken.parse(inStream);
        // Compose a new token.
        return new IdentityToken(sigHeader, latestManifest, null, userToken);
      }
      else { // This token has some intermediate manifests.
        ...
        // Compose a new token.
        return new IdentityToken(sigHeader, latestManifest, priorManifests, userToken);
      }
    }
