 ...
 *          Record.getPacket() and Header.getNextHeader() methods. The type is 
 *          accessible through the Header.getType() method
 */
public class Record {
	private final static String BROADCAST= "0xFFFFFFFFFFFF", CLASS="Record";
	private BitBuf data;
	private Prolog pro;
	private int frmhdrl, datalen, frmtype, ifshdr= 176;
	private Packet packet; // The parsed packet of data
	private Field IFSRECN, 		/* Record number              */
					IFSRCTD, 	/* 64 bit TOD                 */
					IFSRECTP, 	/* Record type                */
					IFSPDULN, 	/* PDU length                 */
					IFSRECST, 	/* Record status              */
					IFSSLT, 	/* SDLC slot                  */
					IFSPORT,	/* SDLC port                  */
	...
	Record(Prolog pro, BitBuf data) {
		...
		if (Trace.isTraceOn() && Trace.isTraceInformationOn()) {
			Trace.log(
				Trace.INFORMATION,
				CLASS + ".Record() " + "Creating Record " + IFSRECN.toString() + "... tcp:" + IFSTCP.toString());
		}
	...
	private void parseIPdata(BitBuf hdr) {
		...
		if (frmtype == 0x86DD) {
			...
			packet.setType(Packet.IP6);
		} else if (frmtype == 0x0800) {
			packet= new IP4Packet(hdr.getBytes());
			packet.setType(Packet.IP4);
		} else if (frmtype == 0x0806) {
			packet= new ARPPacket(hdr.getBytes(), frmtype);
			packet.setType(Packet.ARP);
		} else if (frmtype == 0x8035) {
			packet= new ARPPacket(hdr.getBytes(), frmtype);
			packet.setType(Packet.RARP);
		} else {
			if (Trace.isTraceOn() && Trace.isTraceInformationOn()) {
				Trace.log(Trace.INFORMATION,CLASS + ".parseIPData() " + "Unknown Packet");
			}
			packet= new UnknownPacket(hdr.getBytes());
			packet.setType(Packet.UNK);
		}
	}
	...
	public String toString(FormatProperties filter) {
		...
		if (tcp) { // If its not TCP just return
			...
			if (filter!=null) { // If filter is enabled
				...
				if (starttime == null
					&& endtime == null) {
					// The filter doesn't apply to the two filters for this Record we will continue on
					print= true;
				} else if (
				...
				if (!print) { // Don't print the packet
					if (Trace.isTraceOn() && Trace.isTraceInformationOn()) {
						Trace.log(Trace.INFORMATION,CLASS + ".toString() " + "Record doesn't pass filters. Not printing");
					}
					return "";
				}
			}
