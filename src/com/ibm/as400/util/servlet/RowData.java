...
*  by the <A href="com.ibm.as400.util.servlet.RowMetaData.html">RowMetaData</A> object 
...
public abstract class RowData implements Serializable
{
   ...
   **/
   ...
   {
      if (rows_.isEmpty() || rowIndex < 0)	// position before first row.
      {
         position_ = -1;
         return false;
      }
      else if (rowIndex >= rows_.size())
      {
	 position_ = rows_.size();		// position after last row.
         return false;
      }
      else
      {
   ...
   **/
   ...
   {
      if (!rows_.isEmpty() && position_ == (rows_.size() -1))
         return true;
      else
         return false;
   }
   ...
   **/
   public boolean relative(int numberOfRows)
   {
      ...
      if (rows_.isEmpty() || temp < 0)		// position before first row.
      {
         position_ = -1;
         return false;
      }
      else if (temp >= rows_.size())		// position after last row.
      {
         position_ = rows_.size();
         return false;
      }
