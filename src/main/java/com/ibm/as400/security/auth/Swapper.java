 ...
 * 
 * <p>
 * The <code>swap()</code> methods of this class have as one of their arguments either an AS400 object or a Connection
 * object. The contract of these methods is to swap the profile in use <em>for a specified connection</em>. Here is the
 * usage pattern:
 * 
 * <pre>
 ...
 * 
 * <p>
 * In constrast, the contract of the <code>swap()</code> methods of class {@link ProfileTokenCredential
 * ProfileTokenCredential} is to swap the profile in use for the <em>current thread of execution</em>. They don't swap
 * the profile in use for a specific AS400 object, but rather for any <em>subsequently-created</em> AS400 objects. Here
 * is the usage pattern:
 * 
 * <pre>
 ...
 * 
 * <p>
 * The <code>Swapper.swap()</code> methods are useful for swapping credentials for existing remote connections. The
 * <code>ProfileTokenCredential.swap()</code> methods are useful for swapping the current thread of execution when
 * running natively in an IBM i JVM.
 * <p>
...
public class Swapper
{
    ...
    /**
     * Swaps the profile on the specified JDBC connection. This method calls system API QSYSETP ("Set To Profile
     * Token").
     * 
     * @param connection    A JDBC connection to the IBM i system. Must be an instance of
     *                      {@link com.ibm.as400.access.AS400JDBCConnection} or
     *                      {@link com.ibm.as400.access.AS400JDBCConnectionHandle}.
     * @param newCredential The credential to use for the swap.
     * @exception AS400SecurityException If a security or authority error occurs.
     * @exception IOException            If an error occurs while communicating with the system.
