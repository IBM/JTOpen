 ...
 *
 * <p>
 * This credential does not support all possible behavior for IBM i system profile handles. It is provided to fill a
 * secondary role in support of other credentials when running on the local IBM i system. A profile handle credential
 * provides the ability to store the current thread identity and restore that identity after performing a swap based on
 * another credential (i.e. ProfileTokenCredential).
 *
 * @see AS400Credential
...
public final class ProfileHandleCredential extends AS400Credential
{
    ...
    public boolean equals(Object o)
    {
        ...
        if (!(o instanceof ProfileHandleCredential))
            return false;
        return hashCode() == ((ProfileHandleCredential) o).hashCode();
    }

    /**
     * Returns the actual bytes for the handle as it exists on the IBM i system.
     *
     * @return The handle bytes; null if not set.
     *
     */
    ...
    /**
     * Returns the name of the class providing an implementation for code delegated by the credential that performs
     * native optimization when running on an IBM i system.
     *
     * @return The qualified class name for native optimizations; null if not applicable.
     *
     */
    @Override
    ...
    /**
     * Returns the name of the class providing an implementation for code delegated by the credential when no native
     ...
     * 
     * <p>
     * These are the values initialized prior to accessing host information for or taking action against the credential
     * and not modified thereafter until the credential is destroyed.
     *
     */
    @Override
     ...
     *
     * @exception AS400SecurityException        If an IBM i system security or authentication error occurs.
     *
     * @exception PropertyVetoException         If the change is vetoed.
     *
     * @exception ExtendedIllegalStateException If the token cannot be initialized due to the current state.
     *
     */
     ...
     *
     * <p>
     * This method allows a credential to be constructed based on an existing handle (i.e. previously created using the
     * QSYGETPH system API).
     *
     * <p>
     * This property cannot be changed once a request initiates a connection for the object to the IBM i system.
     *
     * @param bytes The handle bytes.
     *
     * @exception PropertyVetoException            If the change is vetoed.
     *
     * @exception ExtendedIllegalArgumentException If the provided value exceeds the maximum allowed length.
     *
     * @exception ExtendedIllegalStateException    If the property cannot be changed due to the current state.
     *
     */
    ...
    /**
     * Indicates if instances of the class are sufficient by themselves to change the OS thread identity.
     *
     * <p>
     * Typically this behavior is dictated by the type of credential and need not be changed for individual instances.
     * 
     * @return true
     *
     */
