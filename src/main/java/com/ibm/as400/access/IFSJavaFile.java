...
public class IFSJavaFile extends java.io.File implements java.io.Serializable
{
    ...
    public IFSJavaFile()
    {
        this(separator); // Default to the root directory.
    ...
    private IFSJavaFile(String path, String name)
    {
        ...
        try
        {
            String canon = canonicalizeDirectory(path) + name;
            ifsFile_.setPath(replace_ ? canon.replace(separatorChar, AS400_SEPARATOR) : canon);
    ...
    /**
     * Creates a new IFSJavaFile instance for the specified system, from a parent pathname string and a child pathname
     * string.
     *
     * @param system The system that contains the IFSJavaFile.
     * @param path   The file path name where the IFSJavaFile is or will be stored.
     ...
     * <p>
     * The directory argument cannot be null. The constructed IFSJavaFile instance uses the following settings taken
     * from <i>directory</i>: <ul compact>
     * <li>system
     * <li>path
     * </ul>
    ...
    /**
     * Creates a new IFSJavaFile instance for the specified system, from a parent abstract pathname and a child pathname
     * string.
     *
     * @param system    The system that contains the IFSJavaFile.
     * @param directory The directory where the IFSJavaFile is or will be stored.
    ...
    /**
     * Tests whether the application can execute the file denoted by this abstract pathname. This method is supported
     * for IBM i V5R1 and higher. For older releases, it simply returns false. If the user profile has *ALLOBJ special
     * authority (and system is V5R1 or higher), this method always returns true.
     * 
     * @return true if and only if the abstract pathname exists and the application is allowed to execute the file. For
     *         consistency with the behavior of java.io.File on IBM i JVM's: If a security error occurs simply because
     *         the user profile isn't allowed to access the file, the error is traced and false is returned.
     ...
     *
     * @return true if and only if the abstract pathname exists and the application is allowed to read the file. For
     *         consistency with the behavior of java.io.File on IBM i JVM's: If a security error occurs simply because
     *         the user profile isn't allowed to access the file, the error is traced and false is returned.
     * @exception SecurityException If an unanticipated security error occurs.
     **/
     ...
     *
     * @return true if and only if the abstract pathname exists and the application is allowed to write the file. For
     *         consistency with the behavior of java.io.File on IBM i JVM's: If a security error occurs simply because
     *         the user profile isn't allowed to access the file, the error is traced and false is returned.
     * @exception SecurityException If an unanticipated security error occurs.
     **/
     ...
     *
     * <p>
     * In this example, returnCode would be less than <code>0</code> because the path of <code>file</code> is less than
     * the path of <code>file2</code>.
     * 
     * <pre>
     ...
     *
     * <p>
     * In this example, returnCode would be greater than <code>0</code> because the path of <code>file</code> is greater
     * than the path of <code>file2</code>.
     * 
     * <pre>
     ...
     *
     * @return <code>0</code> if this IFSJavaFile path equals <code>file</code>'s path; a value less than <code>0</code>
     *         if this IFSJavaFile path is less than the <code>file</code>'s path; and a value greater than
     *         <code>0</code> if this IFSJavaFile path is greater than the <code>file's</code> path.
     *
     * @since JDK1.2
     **/
     ...
     *
     * @return <code>0</code> if this IFSJavaFile path equals the argument's path; a value less than <code>0</code> if
     *         this IFSJavaFile path is less than the argument's path; and a value greater than <code>0</code> if this
...
//  }
     ...
     *
     * @return <code>true</code> if the file is successfully deleted; <code>false</code> otherwise.
    ...
    /**
     * Compares this IFSJavaFile against the specified object. Returns <code>true</code> if and only if the argument is
     ...
     *
     * @return <code>true</code> if the file specified by this object exists; <code>false</code> otherwise.
    ...
    /**
     * Returns An IFSJavaFile object based on the absolute path name of the current object. If the <tt>system</tt>
    ...
    /**
     * Returns An IFSJavaFile object based on the canonical path name of the current object. If the <tt>system</tt>
     * property is set, it is copied to the returned object.
     *
     ...
     * <p>
     * The returned number of unallocated bytes is a hint, but not a guarantee, that it is possible to use most or any
     * of these bytes. The number of unallocated bytes is most likely to be accurate immediately after this call. It is
     * likely to be made inaccurate by any external I/O operations including those made on the system outside of this
     * virtual machine. This method makes no guarantee that write operations to this file system will succeed.
     * 
     * @return The number of unallocated bytes on the partition; or 0L if the abstract pathname does not name a
     *         partition. This value will be less than or equal to the total file system size returned by
     *         getTotalSpace().
     * @exception SecurityException If the user is denied access to the file.
     **/
    @Override
     ...
     * When possible, this method checks for write permissions and other operating system restrictions and will
     * therefore usually provide a more accurate estimate of how much new data can actually be written than
     * getFreeSpace().
     * <p>
     * The returned number of available bytes is a hint, but not a guarantee, that it is possible to use most or any of
     * these bytes. The number of unallocated bytes is most likely to be accurate immediately after this call. It is
     * likely to be made inaccurate by any external I/O operations including those made on the system outside of this
     * virtual machine. This method makes no guarantee that write operations to this file system will succeed.
     * <p>
     * Note: If the user profile has a "maximum storage allowed" setting of *NOMAX, then getUsableSpace() returns the
     * same value as {@link #getFreeSpace getFreeSpace()}.
     * 
    ...
    /**
     * Returns the name of the IFSJavaFile. The name is everything in the path name after the last occurrence of the
     * separator character.
     * <p>
    ...
    /**
     * Returns the parent directory of the IFSJavaFile. The parent directory is everything in the path name before the
     * last occurrence of the separator character, or null if the separator character does not appear in the path name.
     * <p>
     * The following example demonstrates the use of this method:
     * <p>
    ...
    /**
     * Returns an IFSJavaFile object that represents the parent of the current object. The parent is the path name
     * before the last occurrence of the separator character. Null is returned if the separator character does not
     * appear in the path or the current object is the file system root. If the <tt>system</tt> property is set, it is
     * also copied to the returned object.
     *
     ...
     * 
     * @return Either {@link IFSFile#PATTERN_POSIX PATTERN_POSIX}, {@link IFSFile#PATTERN_POSIX_ALL PATTERN_POSIX_ALL},
     *         or {@link IFSFile#PATTERN_OS2 PATTERN_OS2}
     ...
     *
     * @return The length, in bytes, of the IFSJavaFile, or <code>0</code>((IFSJavaFile) if it does not exist.
     * @exception SecurityException If the user is denied access to the file.
     **/
    @Override
     ...
     *         the current directory or the parent directory. If this IFSJavaFile is not a directory, null is returned.
     *         If this IFSJavaFile is an empty directory, or the file name filter does not match any files, an empty
     *         string array is returned. The IFSFile object passed to the file name filter object have cached file
     ...
     * <p>
     * Note:<br>
     * If the file does not match <i>pattern</i>, it will not be processed by <i>filter</i>.
     *
     * @param filter  The file name filter.
    ...
    /**
     * Lists the files in this IFSJavaFile directory that satisfy <i>filter</i>. With the use of this function,
    ...
    public File[] listFiles(FilenameFilter filter) throws SecurityException
    {
        IFSFile[] files = null;
        try {
            files = ifsFile_.listFiles0(null, "*");
        }
        catch (AS400SecurityException e) {
            Trace.log(Trace.ERROR, e);
            throw new SecurityException(e);
     ...
     *
     * @return An array of objects in the directory that satisfy the file filter. This list does not include the current
     *         directory or the parent directory. If this IFSJavaFile is not a directory, null is returned. If this
     *         IFSJavaFile is an empty directory, or the file filter does not match any files, an empty object array is
     *         returned. The IFSJavaFile object passed to the file filter object has cached file attribute information.
     *         Maintaining references to these IFSJavaFile objects after the list operation increases the chances that
    ...
    /**
     * Lists the files in the IFSJavaFile directory that satisfy <i>file name filter</i>. With the use of this function,
     * attribute information is cached and will not be refreshed from the IBM i system. This means attribute information
     * may become inconsistent with the IBM i system.
     * 
     * @param filter The file name filter.
     ...
     *         current directory or the parent directory. If this IFSJavaFile is not a directory, null is returned. If
     *         this IFSJavaFile is an empty directory, or the file name filter does not match any files, an empty object
     *         array is returned. The IFSFile object passed to the file name filter object has cached file attribute
     *         information. Maintaining references to these IFSFile objects after the list operation increases the
     *         chances that their file attribute information will not be valid.
     *         <p>
     *         The following example demonstrates the use of this method:
     * 
    ...
    /**
     * Lists the files in this IFSJavaFile directory that satisfy <i>filter</i> and <i>pattern</i>. With the use of this
     * function, attribute information is cached and will not be refreshed from the IBM i system. This means attribute
     * information may become inconsistent with the IBM i system.
     *
     * <p>
     * Note:<br>
     * If the file does not match <i>pattern</i>, it will not be processed by <i>filter</i>.
    ...
    /**
     * Lists the files in this IFSJavaFile directory that match <i>pattern</i>. With the use of this function, attribute
     ...
     *
     * 
     * @return <code>true</code> if the directory could be created; <code>false</code> otherwise.
     * @exception SecurityException If the user is denied access to the file.
     **/
    @Override
     ...
     *
     * @return <code>true</code> if the directory (or directories) could be created; <code>false</code> otherwise.
     * @exception SecurityException If the user is denied access to the file.
     **/
    @Override
    ...
    /**
     * Renames the IFSJavaFile to have the path name of <i>dest</i>. Wildcards are not permitted in this file name.
     *
     * @param dest The new filename.
     *
     * @return <code>true</code> if the renaming succeeds; <code>false</code> otherwise.
     * @exception SecurityException If the user is denied access to the file.
     **/
     ...
     *
     * @return <code>true</code> if the path was set; <code>false</code> otherwise.
     **/
    ...
    /**
     * Sets the pattern-matching behavior used when files are listed by any of the <tt>list()</tt> or
     * <tt>listFiles()</tt> methods. The default is PATTERN_POSIX.
     * 
     ...
     *
     * @return <code>true</code> if the system was set; <code>false</code> otherwise.
     **/
     ...
     * <p>
     * An invocation of this method of the form <tt>file.setExcutable(arg)</tt> behaves in exactly the same way as the
     * invocation <tt>file.setExecutable(arg, true)</tt>
     * 
     * @param executable If true, sets the access permission to allow execute operations; if false, to disallow execute
     *                   operations.
    ...
    /**
     * Sets the owner's or everybody's execute permission for this abstract pathname. This method is supported for IBM i
     * V5R1 and higher. For older releases, it simply returns false.
     * 
     * @param executable If true, sets the access permission to allow execute operations; if false, to disallow execute
     *                   operations.
     * @param ownerOnly  If true, the execute permission applies only to the owner's execute permission; otherwise, it
    ...
    /**
     * A convenience method to set the owner's read permission for this abstract pathname. This method is supported for
     * IBM i V5R1 and higher. For older releases, it simply returns false.
     * <p>
     * An invocation of this method of the form <tt>file.setReadable(arg)</tt> behaves in exactly the same way as the
     * invocation <tt>file.setReadable(arg, true)</tt>
     * 
     * @param readable If true, sets the access permission to allow read operations; if false, to disallow read
     *                 operations.
     * @return true if and only if the operation succeeded. The operation will fail if the user does not have permission
     *         to change the access permissions of this abstract pathname.
     * @exception SecurityException If the user is denied access to the file.
    ...
    /**
     * Sets the owner's or everybody's read permission for this abstract pathname. This method is supported for IBM i
     * V5R1 and higher. For older releases, it simply returns false.
     * 
     * @param readable  If true, sets the access permission to allow read operations; if false, to disallow read
     *                  operations.
     * @param ownerOnly If true, the read permission applies only to the owner's read permission; otherwise, it applies
     *                  to everybody.
     * @return true if and only if the operation succeeded. The operation will fail if the user does not have permission
     *         to change the access permissions of this abstract pathname.
     * @exception SecurityException If the user is denied access to the file.
    ...
    /**
     * A convenience method to set the owner's write permission for this abstract pathname. This method is supported for
     * IBM i V5R1 and higher. For older releases, it simply returns false.
     * <p>
     * An invocation of this method of the form <tt>file.setWritable(arg)</tt> behaves in exactly the same way as the
     * invocation <tt>file.setWritable(arg, true)</tt>
     * 
    ...
    /**
     * Converts the abstract path name into a <code>file:</code> URL. The IBM i file/directory will be accessed and if
