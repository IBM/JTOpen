 ...
 * {@link AS400 AS400} class uses a SignonHandler if additional information
 * (such as userID or password) must be gathered at runtime when connecting to
 * the server.
 **/
final class ToolboxSignonHandler extends SignonHandlerAdapter
{
    ...
    /**
     * Gathers any missing sign-on information. If GUI is available, displays an
     * interactive sign-on dialog to the user, with fields for systemName, userID,
     * and password, along with "OK" and "CANCEL" buttons. Otherwise, simply returns
     * <tt>true</tt>.
     * 
     * @param event       The sign-on event.
     * @param forceUpdate <tt>true</tt> indicates that the sign-on information is
     *                    known to be incomplete or incorrect. <tt>false</tt>
     *                    indicates that the information may be correct.
    ...
    /**
     * Handles an exception that was thrown during a sign-on attempt. If the handler
     * cannot deal with the exception, the handler rethrows <tt>exc</tt>.
     * 
     * @param event The sign-on event.
    ...
    /**
     * Displays a message warning that the password will expire in
     * <tt>daysUntilExpiration</tt> days, and asks the userif they would like to
     * change the password. If user clicks OK, then a "change password" dialog is
     * displayed, prompting user for old and new passwords. Otherwise, simply
     * returns true.
     * 
     * @param event The sign-on event.
     * @return true
    ...
    public boolean passwordAboutToExpire(SignonEvent event, int daysUntilExpiration)
    {
        ...
        boolean response = displayMessage(ResourceBundleLoader.substitute(ResourceBundleLoader.getText("DLG_PASSWORD_EXP_WARNING"), Integer.toString(daysUntilExpiration)) + "  " + ResourceBundleLoader.getText("DLG_CHANGE_PASSWORD_PROMPT"), 
                                          ResourceBundleLoader.getText("DLG_SIGNON_TITLE"), true);
        if (response)
    ...
    /**
     * Displays a message indicating that the password has expired, and asks the
     * user if they would like to change the password. If user clicks OK, then a
     * "change password" dialog is displayed, prompting user for old and new
     * passwords.
     * 
     * @param event The sign-on event.
    ...
    public boolean passwordExpired(SignonEvent event)
    {
        ...
        // See if user wants to change password, if so, display change password panel.
        boolean response = displayMessage(ResourceBundleLoader.getText("EXC_PASSWORD_EXPIRED") + "\n" + ResourceBundleLoader.getText("DLG_CHANGE_PASSWORD_PROMPT"), 
                                          ResourceBundleLoader.getText("DLG_SIGNON_TITLE"), true);
        if (response)  
    ...
    /**
     * Displays a message indicating that the password is incorrect, and redisplays
     * the sign-on dialog.
     * 
     * @param event The sign-on event.
     * @return true if user clicks OK, false if user clicks Cancel.
    ...
    /**
     * Displays a message indicating that the password length is incorrect, and
     * redisplays the sign-on dialog.
     * 
     * @param event The sign-on event.
     * @return true if user clicks OK, false if user clicks Cancel.
     **/
    ...
    /**
     * Displays a message indicating that the password was not specified, and
     * redisplays the sign-on dialog.
     * 
     * @param event The sign-on event.
     * @return true if user clicks OK, false if user clicks Cancel.
    ...
    /**
     * Displays a message indicating that the system name was not specified, and
     * redisplays the sign-on dialog.
     * 
     * @param event The sign-on event.
     * @return true if user clicks OK, false if user clicks Cancel.
    ...
    // Design Note: In order to preserve historical default behavior, this class does not implement the systemNameUnknown() method. 
    // If an unknown system name is specified, an UnknownHostException will be thrown.


    /**
     * Displays a message indicating that the user profile will be disabled if next
     * sign-on attempt is incorrect, and redisplays the sign-on dialog.
     * 
     * @param event The sign-on event.
     * @return true if user clicks OK, false if user clicks Cancel.
     **/
    ...
    /**
     * Displays a message warning that the system object already has a default user
     * assigned.
     * 
     * @param event       The sign-on event.
    ...
    /**
     * Displays a message indicating that the user profile is disabled, and
     * redisplays the sign-on dialog.
     * 
     * @param event The sign-on event.
     * @return true if user clicks OK, false if user clicks Cancel.
    ...
    /**
     * Displays a message indicating that the user ID length is incorrect, and
     * redisplays the sign-on dialog.
     * 
     * @param event The sign-on event.
     * @return true if user clicks OK, false if user clicks Cancel.
    ...
    /**
     * Displays a message indicating that the user name was not specified, and
     * redisplays the sign-on dialog.
     * 
     * @param event The sign-on event.
     * @return true if user clicks OK, false if user clicks Cancel.
    ...
    /**
     * Displays a message indicating that the specified user ID is unknown, and
     * redisplays the sign-on dialog.
     * 
     * @param event The sign-on event.
     * @return true if user clicks OK, false if user clicks Cancel.
     ...
     * @return The response from the user. true if user indicated approval, false
     *         otherwise.
     **/
    ...
    private void handleException(AS400 system, AS400SecurityException exc) throws AS400SecurityException
    {
        switch (exc.getReturnCode())
        {
          ...
          case AS400SecurityException.PASSWORD_NEW_VALIDATION_PROGRAM:
              ...
              { 
                  // not changing password, Give user another chance on the sign-on dialog.
          ...
          case AS400SecurityException.PASSWORD_EXPIRED:
              ...
              {  
                  // User canceled the password change. Fail the sign-on.
                  throw exc;
              }
          ...
          default:  
              // None of the above.  Just display the error text to the user and throw the exception.
              String title = (changingPassword_) ? ResourceBundleLoader.getText("DLG_CHANGE_PASSWORD_TITLE") : ResourceBundleLoader.getText("DLG_SIGNON_TITLE");
    ...
    /**
     * Gathers any missing sign-on information, and calls the relevant setters in
     * the system object. If GUI is available, displays an interactive sign-on
     * dialog to the user, with fields for systemName, userID, and password, along
     * with "OK" and "CANCEL" buttons. Otherwise, simply returns <tt>true</tt>.
     * 
     * @param event                 The sign-on event.
     * @param forceUpdate           <tt>true</tt> indicates that the sign-on
     *                              information is known to be incomplete or
     *                              incorrect. <tt>false</tt> indicates that the
     *                              information may be correct.
     * @param enableSystemNameField Indicates whether the "system name" field should
            ...
            do
            {
                ...
                if (validateInfo(systemName, userId, password, system))
                {
                    try
                    {
                        ...
                        // erroneous userID is specified. Then that userID gets set as the default user.
                        // To reset it, the app must first call AS400.removeDefaultUser().
