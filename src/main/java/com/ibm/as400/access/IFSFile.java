...
public class IFSFile implements Serializable, Comparable
{
   ...
   * <i>system</i> that has a path name of <i>directory</i>, that is followed by the separator character and
   * <i>name</i>.
   * 
  ...
  /**
   * Constructs an IFSFile object. It creates an IFSFile instance that represents the integrated file system object on
   * <i>system</i> that has a path name of <i>path</i>.
   * 
   * @param system The system that contains the file.
  ...
  /**
   * Constructs an IFSFile object. It creates an IFSFile instance that represents the integrated file system object on
   * <i>system</i> that has a path name is of <i>directory</i>, followed by the separator character and <i>name</i>.
   * 
   * @param system    The system that contains the file.
   ...
   * <p>
   * The <tt>directory</tt> argument cannot be null. The constructed IFSFile instance uses the following settings taken
   * from <tt>directory</tt>:
   * <ul>
   * <li>system
   * <li>path
   * </ul>
   * The resulting file name is taken from the path name of <i>directory</i>, followed by the separator character,
   * followed by <i>name</i>.
   *
  ...
  public IFSFile(AS400 system, IFSJavaFile directory, String  name)
  {
      ...
      initialize(system, directory.getAbsolutePath().replace(File.separatorChar, separatorChar), name);
  }
  ...
  private void initialize(AS400 system, String directory, String name)
  {
      ...
      path_   = normalizePath(directory, name);
      system_ = system;
  }
   ...
   * Constructs an IFSFile object. It creates an IFSFile instance that represents the integrated file system object on
   * <i>system</i> that has a path name of <i>directory</i>, that is followed by the separator character and
   * <i>name</i>. {@link #isDirectory() isDirectory} and {@link #isFile() isFile} will both return false for invalid
   * symbolic links.
   * 
   * @param system     The system that contains the file.
  ...
  IFSFile(AS400 system, IFSCachedAttributes attributes)
  {
      ...
      initialize(system, attributes.getParent(), attributes.getName());

      // Cache file attributes.
      cachedAttributes_ = attributes;
   
      //isDirectory and isFile can both be false if the object is an invalid symbolic link.
   ...
   * This method is supported for IBM i V5R1 and higher. For older releases, it simply returns false. If the user
   * profile has *ALLOBJ special authority (and system is V5R1 or higher), this method always returns true.
   * 
  ...
  /**
   * Determines if the application can read from the integrated file system object represented by this object. Note that
   * IBM i <i>directories</i> are never readable; only <i>files</i> can be readable.
   * 
  ...
  /**
   * Determines if the application can write to the integrated file system object represented by this object. Note that
   * IBM i <i>directories</i> are never writable; only <i>files</i> can be writable.
   * 
  ...
  void chooseImpl() throws IOException, AS400SecurityException
  {
    ...
    impl_ = (IFSFileImpl) system_.loadImpl2("com.ibm.as400.access.IFSFileImplRemote",
  ...
  public void clearCachedAttributes()
  {
      ...
      if (impl_ != null)
          impl_.clearCachedAttributes();
  }

  /**
   * Compares the path of this IFSFile with an <code>Object</code>'s path. If the other object is not an IFSFile or
   * java.io.File, this method throws a <code>ClassCastException</code>, since IFSFile is comparable only to IFSFile and
   * java.io.File.
   *
   * <p>
   * Note:<br>
   ...
   *
   * @return <code>0</code> if this IFSFile path equals the argument's path; a value less than <code>0</code> if this
   *         IFSFile path is less than the argument's path; and a value greater than <code>0</code> if this IFSFile path
   ...
   * </ul>
   * 
   * @param path The destination path to copy this IFSFile to. <br>
   *             If the system is V5R2 or earlier: If the current object is a file (rather than a directory), the
   *             destination path must also specify a file (rather than a directory), otherwise the copy may fail.
   * @return true if the copy succeeded (or at least one file of a source directory's contents was copied); false
   *         otherwise.
   * @throws IOException                  If an error occurs while communicating with the system.
  ...
  **/
   ...
   * 
   * @return The time (measured in milliseconds since 01/01/1970 00:00:00 GMT) that the integrated file system object
   *         was created, or 0L if the object does not exist or is not accessible.
   * @throws IOException If an error occurs while communicating with the system.
   * 
   * @exception ConnectionDroppedException If the connection is dropped unexpectedly.
   * @exception ExtendedIOException        If an error occurs while communicating with the system.
  ...
  public long created() throws IOException
  {
      try
      {
          if (cachedAttributes_ != null)
              return cachedAttributes_.getCreationDate();
   ...
   * operation that is atomic with respect to all other filesystem activities that might affect the file. If the file
   * already exists, its data are left intact.
   * 
   * @return <tt>true</tt> if the named file does not exist and was successfully created; <tt>false</tt> if the named
   *         file already exists.
   * 
   * @exception ConnectionDroppedException If the connection is dropped unexpectedly.
   * @exception ExtendedIOException        If an error occurs while communicating with the system.
   ...
   * 
   * @return true if the file system object is successfully deleted; false otherwise. Returns false if the file system
   *         object did not exist prior to the delete() or is not accessible.
   * @throws IOException If an error occurs while communicating with the system.
   * 
   * @exception ConnectionDroppedException If the connection is dropped unexpectedly.
   * @exception ExtendedIOException        If an error occurs while communicating with the system.
  ...
  public boolean delete() throws IOException
  {
      ...
      if(isInQsys())
      {
          ...
          try 
          {
              ...
              Trace.log(Trace.WARNING, "Unable to delete QSYS file using classic techniques. Will try host server stream file operations."+path_, e);
          }
          finally
          {
              if(null != tmpPF)
   ...
   * The returned Enumeration contains an IFSFile object for each file or directory in the list. The list is loaded
   * incrementally, which will improve initial response time for large lists.
   * 
   ...
   *                marks (?).
   * 
   * @return An Enumeration of IFSFile objects which represent the contents of the directory that satisfy the filter and
   *         pattern. This Enumeration does not include the current directory or the parent directory. If this object
   *         does not represent a directory, this object represents an empty directory, or the filter or pattern does
   *         not match any files, then an empty Enumeration is returned. The IFSFile object passed to the filter object
   *         has cached file attribute information.
   * @throws IOException If an error occurs while communicating with the system.
   * 
   * @exception ConnectionDroppedException If the connection is dropped unexpectedly.
   ...
   * @exception UnknownHostException       If the system cannot be located.
   **/
  public Enumeration<IFSFile> enumerateFiles(IFSFileFilter filter, String pattern) throws IOException
  {
  ...
  /**
   * Lists the integrated file system objects in the directory represented by this object. The returned Enumeration
   * contains an IFSFile object for each file or directory in the list. The list is loaded incrementally, which will
   * improve initial response time for large lists.
   * 
   * @param pattern The pattern that all filenames must match. Acceptable characters are wildcards (*) and question
   *                marks (?).
   * 
   * @return An Enumeration of IFSFile objects which represent the contents of the directory. This Enumeration does not
   *         include the current directory or the parent directory. If this object does not represent a directory, this
   *         object represents an empty directory, or the pattern does not match any files, then an empty Enumeration is
   *         returned.
   * @throws IOException If an error occurs while communicating with the system.
   * 
   * @exception ConnectionDroppedException If the connection is dropped unexpectedly.
   ...
   * @exception UnknownHostException       If the system cannot be located.
   **/
  public Enumeration<IFSFile> enumerateFiles(String pattern) throws IOException
  {
      // Validate arguments.  Note that we tolerate a null-valued 'filter'.
      if (pattern == null)
          throw new NullPointerException("pattern");
   ...
   * @return An Enumeration of IFSFile objects which represent the contents of the directory. This Enumeration does not
   *         include the current directory or the parent directory. If this object does not represent a directory, this
   *         object represents an empty directory, or the filter does not match any files, then an empty Enumeration is
   *         returned. The IFSFile object passed to the filter object has cached file attribute information.
   * @throws IOException If an error occurs while communicating with the system.
   * 
   * @exception ConnectionDroppedException If the connection is dropped unexpectedly.
   * @exception ExtendedIOException        If an error occurs while communicating with the system, or the directory is
   ...
   * @exception UnknownHostException       If the system cannot be located.
   **/
  public Enumeration<IFSFile> enumerateFiles(IFSFileFilter filter) throws IOException
  {
      try {
   ...
   * @return An Enumeration of IFSFile objects which represent the contents of the directory. This Enumeration does not
   *         include the current directory or the parent directory. If this object does not represent a directory, this
   *         object represents an empty directory, or the filter or pattern does not match any files, then an empty
   ...
   * @exception UnknownHostException       If the system cannot be located.
   **/
  public Enumeration<IFSFile> enumerateFiles() throws IOException
  {
      try {
   ...
   * @return true if the path name and system names of the objects are equal; false otherwise.
   **/
  @Override
  ...
  /**
   * Returns the path name of the integrated file system object represented by this object. This is the full path
  ...
  /**
   * Returns the canonical pathname string of the integrated file system object represented by this object. This is the
   * full path starting at the root directory. This typically involves removing redundant names such as "." and ".."
   * from the pathname. Symbolic links are not resolved.
   * 
   * @return The canonical path name of this integrated file system object.
   **/
  public String getCanonicalPath()
  {
      // Numerous changes added to remove DOT, DOTDOT, and multiple
      // separator characters.  Previously, this method simply returned path_   
      ...
      // Return path.  Note that 0-length path is possible if path_ contains nothing but DOTs and DOTDOTs
      return (pathBuffer.length() == 0) ? separator : pathBuffer.toString();
  }
   ...
   * authentication scheme is not password, returns -1.
   * 
  ...
  /**
   * Returns the description of the object.  Not all objects have descriptions, in which 
   * case a zero-length string is returned. 
   * 
   * @return description of the object. 
   * @throws IOException  IOException If an error occurs while communicating with the system.
   */
   ...
   * 
   * @return The number of bytes of storage available to the user, or special value {@link Long#MAX_VALUE
   *         Long.MAX_VALUE} if the system reports "no maximum".
   * @throws IOException If an error occurs while communicating with the system.
   * 
   * @exception ConnectionDroppedException If the connection is dropped unexpectedly.
   * @exception ExtendedIOException        If an error occurs while communicating with the system.
  ...
  public static long getFreeSpace(AS400 system) throws IOException
  {
      ...
      try
      {
          IFSFileImpl impl = (IFSFileImpl) system.loadImpl2("com.ibm.as400.access.IFSFileImplRemote",
                                                            "com.ibm.as400.access.IFSFileImplProxy");
   ...
   * INTERNAL USE ONLY - SUBJECT TO CHANGE
   * 
   * Returns the amount of unused storage space that is available to the user. Note: If the user profile has a "maximum
   * storage allowed" setting of *NOMAX, then getAvailableSpace(true) returns the same value as
   * getAvailableSpace(false).
   * 
   * @param forUserOnly Whether to report only the space for the user. If false, report space available in the entire
   *                    file system.
   ...
   * 
   * @param forUserOnly Whether to report only the space for the user. If false, report total space in the entire file
   *                    system.
   ...
   * INTERNAL USE ONLY - SUBJECT TO CHANGE
   * 
   * Package scope getter methods to return data set by listFiles0()  
   *
   * Number of objects returned  by the File Server to listFiles0() and   
   * also the restartName and restartID info of the last object returned.  
   */
  int getListFiles0LastNumObjsReturned()  {                                            
      return listFiles0LastNumObjsReturned_;     
  }     
  
  String getListFiles0LastRestartName() {                                            
      return (listFiles0LastRestartName_ == null) ?  "" : listFiles0LastRestartName_;       
  }    
  ...
  byte[] getListFiles0LastRestartID() {                                            
      return listFiles0LastRestartID_;           
  }                                            
  ...
  public String getName()
  {
      ...
      // If name contains exactly 1 quote, and there's another quote earlier in the path, back up to the separator
      // character before the prior quote.
   ...
   * 
   * @return The name of the user profile that owns the file, or "" if owner cannot be determined. If the file is a
   *         directory, only password authentication scheme is supported.
   * 
  ...
  public String getOwnerName() throws IOException, AS400SecurityException
  {
      ...
      if (!isFile())
          return (isDirectory()) ? impl_.getOwnerNameByUserHandle(false) : impl_.getOwnerNameByUserHandle(true);
   ...
   * 
   * @return The file owner's ID number, or -1 if the file is non-existent or is a directory.
   * @exception IOException If an error occurs while communicating with the system.
   **/
  public long getOwnerUID() throws IOException
  {
      // Design note: It would be preferable if we could report the user profile name instead of UID number.  
      // However, the File Server doesn't report the user name.  FYI: There is a service program (QSYPAPI/getpwuid()) 
      // that reports the user profile name associated with a given uid number, but getpwuid returns its results 
      // as a pointer to a struct, which our ServiceProgramCall class can't handle at the moment.
      long result = -1L;
      try
      {
          if (impl_ == null)
  ...
  public int getOwnerId() throws IOException {
      return (int)getOwnerUID();
  }

  /**
   * Returns the path of the parent directory of the integrated file system object represented by this object. The
   * parent directory is everything in the path name before the last occurrence of the separator character, or null if
   * the separator character does not appear in the path name.
   * 
  ...
  /**
   * Returns the parent directory of the current object. The parent is the path name before the last occurrence of the
   * separator character. Null is returned if the separator character does not appear in the path, or if the current
   * object is the file system root. If the <tt>system</tt> property is not yet set in the current object, then the
   * <tt>system</tt> property will not be set in the returned object.
   * 
   * @return an IFSJavaFile object representing the parent directory if one exists; null otherwise.
   * 
   * @see #getParent()
   **/
   ...
   * by this object; or <tt>null</tt> if the file is not a symbolic link or does not exist.
   * <p>
  ...
  public String getPathPointedTo() throws IOException, AS400SecurityException
  {
      ...
      if (cachedAttributes_ != null && !isSymbolicLink_) 
          return null;
  ...
  /**
   * Returns the pattern-matching behavior used when files are listed by any of the <tt>list()</tt> or
   * <tt>listFiles()</tt> methods. The default is PATTERN_POSIX.
   * 
   * @return Either {@link #PATTERN_POSIX PATTERN_POSIX}, {@link #PATTERN_POSIX_ALL PATTERN_POSIX_ALL}, or
   *         {@link #PATTERN_OS2 PATTERN_OS2}
   * @throws IOException If an error occurs while communicating with the system.
  ...
  byte[] getRestartID()
  {
      ...
      if (Trace.traceOn_) Trace.log(Trace.ERROR, "IFSFile.getRestartID() was called when cachedAttributes_==null.");
      return null;
  }

  /**
   * Returns the subtype of the integrated file system object represented by this object. Some possible values that
   * might be returned include:<br>
   * CMNF, DKTF, DSPF, ICFF, LF, PF, PRTF, SAVF, TAPF.<br>
   * Note that many file system objects do not have a subtype: for example, <tt>.MBR</tt> objects, and any Root,
   * QOpenSys or UDFS object.
   * 
   * @return The subtype of the object, or a zero-length string if the object has no subtype.
   * @throws IOException If an error occurs while communicating with the system.
   * 
   * @exception ExtendedIOException    If an error occurs while communicating with the system.
   * @exception AS400SecurityException If a security or authority error occurs.
  ...
  private boolean isInFileSystem(String fs)
  {
      String lowercasePath = path_.toLowerCase().replaceAll("//+","/");
      return lowercasePath.equals("/"+fs.toLowerCase()) || lowercasePath.startsWith("/"+fs.toLowerCase()+"/");
  }
  ...
  /**
   * Determines if the file is an IBM i "source physical file". Physical files reside under QSYS, and can be either
   * source files (type *SRC) or data files (type *DATA). For further information, refer to the specification of the
   * QDBRTVFD (Retrieve Database File Description) API.
   * 
  ...
  public boolean isSourcePhysicalFile() throws AS400Exception, AS400SecurityException, IOException
  {
      ...
      if (!pathUpperCase.endsWith(".FILE") || pathUpperCase.indexOf("/QSYS.LIB") == -1 || !getSubtype().equals("PF"))
      {
   ...
   * @return Return the auxiliary storage pool (ASP) that holds the object. Possible values are:
   *         <ul>
   ...
   *         <li>-1: unable to retrieve ASP.
   *         </ul>
   * @throws IOException            If an error occurs while communicating with the system.
   * @throws AS400SecurityException If a security or authority error occurs.
   * @deprecated Use {@link #getASP()}
   **/
   ...
   * 
   * @param retrieveAll true or false, retrieve all attributes at same time.
   * @return Return the type of file system. Possible values
   *         are: EPFS,QDLS,QSYS,NFS,LRFS,QOPT,QRFS,EPFSP,QNETC,QDTL,IEPFS, and ASPQSYS. 
  ...
  public String getFileSystemType() throws IOException, AS400SecurityException {
      return getFileSystemType(false);
  }
  ...
  private boolean isDirectoryInited_ = false;
  ...
  /**
   * Determines if the integrated file system object represented by this object is a directory. <br>
   * Both isDirectory() and {@link #isFile() isFile} will return false for invalid symbolic links.
   * 
   ...
   * Both {@link #isDirectory() isDirectory} and isFile() will return false for invalid symbolic links.
   * 
   ...
   * 
   * @return true if the hidden attribute of this integrated file system object is set; false otherwise.
   * @throws IOException If an error occurs while communicating with the system.
   * 
   * @exception AS400SecurityException     If a security or authority error occurs.
   * @exception ConnectionDroppedException If the connection is dropped unexpectedly.
  ...
  public boolean isHidden() throws IOException, AS400SecurityException
  {
      ...
      if (cachedAttributes_ != null)
          return (cachedAttributes_.getFixedAttributes() & IFSCachedAttributes.FA_HIDDEN) != 0;
  ...
  /**
   * Determines if the integrated file system object represented by this object is a symbolic link. <br>
   * Note: Both {@link #isDirectory() isDirectory} and {@link #isFile() isFile} resolve symbolic links to their ultimate
   * destination. For example, if this object represents a symbolic link on the system, that resolves to a file object,
   * then isSymbolicLink() will return true, isFile() will return true, and isDirectory() will return false. <br>
   * Note: If the system is V5R2 or earlier, this method always returns false, regardless of whether the system object
   * is a link or not.
   * 
   * @return true if the specified file exists and is a symbolic link; false otherwise. <br>
   ...
   * 
   * @return true if the read only attribute of this integrated file system object is set; false otherwise.
   * @throws IOException If an error occurs while communicating with the system.
   * 
   * @exception AS400SecurityException     If a security or authority error occurs.
   * @exception ConnectionDroppedException If the connection is dropped unexpectedly.
  ...
  public boolean isReadOnly() throws IOException, AS400SecurityException
  {
      ...
      // use the cached attributes instead of getting a new set from the system.
      if (cachedAttributes_ != null)
          return (cachedAttributes_.getFixedAttributes() & IFSCachedAttributes.FA_READONLY) != 0;
  ...
  /**
   * Returns the sorting behavior used when files are listed by any of the <tt>list()</tt> or <tt>listFiles()</tt>
   * methods.
   * 
  ...
  /**
   * Determines the time that the integrated file system object represented by this object was last accessed. With the
   * use of the {@link #listFiles() listFiles} methods, attribute information is cached and will not be automatically
   * refreshed from the system. This means the reported last accessed time may become inconsistent with the system.
   * 
   * @return The time (measured in milliseconds since 01/01/1970 00:00:00 GMT) that the integrated file system object
   *         was last accessed, or 0L if the object does not exist or is not accessible.
   * @throws IOException If an error occurs while communicating with the system.
   * 
   * @exception ConnectionDroppedException If the connection is dropped unexpectedly.
   * @exception ExtendedIOException        If an error occurs while communicating with the system.
  ...
  {                                                      
      ...
      {
          if (cachedAttributes_ != null)
              return cachedAttributes_.getAccessDate();

          if (impl_ == null)
              chooseImpl();
          
          return impl_.lastAccessed();
      }                                              
      catch (AS400SecurityException e)                     
      {                                                    
          Trace.log(Trace.ERROR, SECURITY_EXCEPTION, e);     
  ...
  /**
   * Determines the time that the integrated file system object represented by this object was last modified. With the
   * use of the {@link #listFiles() listFiles} methods, attribute information is cached and will not be automatically
   * refreshed from the system. This means the reported last modified time may become inconsistent with the system.
   * 
   * @return The time (measured in milliseconds since 01/01/1970 00:00:00 GMT) that the integrated file system object
   *         was last modified, or 0L if it does not exist or is not accessible.
   * @throws IOException If an error occurs while communicating with the system.
   * 
   * @exception ConnectionDroppedException If the connection is dropped unexpectedly.
   * @exception ExtendedIOException        If an error occurs while communicating with the system.
  ...
  /**
   * Determines the length of the integrated file system object represented by this object. With the use of the
   * {@link #listFiles() listFiles} methods, attribute information is cached and will not be automatically refreshed
   * from the system. This means the reported length may become inconsistent with the system.
   * 
   * @return The length, in bytes, of the integrated file system object, or 0L if it does not exist or is not
   *         accessible. <br>
   *         <b>Note:</b> When used following {@link #listFiles() listFiles} or {@link #enumerateFiles() enumerateFiles}
   *         methods (for symbolic link objects) this method will return the length of the symbolic link object as
   *         returned by the File Server. In order to retrieve the length of the <b>target</b> object, you must use
   ...
   *         directory. If this object does not represent a directory, null is returned. If this object represents an
   *         empty directory, an empty string array is returned. <br>
   ...
   * @return An array of object names in the directory that satisfy the filter. This list does not include the current
   *         directory or the parent directory. If this object does not represent a directory, null is returned. If this
   *         object represents an empty directory, or the filter does not match any files, an empty string array is
   *         returned. The IFSFile object passed to the filter object have cached file attribute information.
   *         Maintaining references to these IFSFile objects after the list operation increases the chances that their
   *         file attribute information will not be valid. <br>
   *         Note: Due to a limitation in the File Server, at most 65,535 files will be listed.
   * @throws IOException If an error occurs while communicating with the system.
   * 
      ...
      if (allNames != null)
      {
          ...
          for (int i = 0; i < allNames.length; i++)
          {
              ...
              if (!(name.equals(".") || name.equals("..")))
              {
                  ...
                  // Specified the add in only one place to avoid confusion
   ...
   *         the chances that their file attribute information will not be valid. <br>
   *         Note: Due to a limitation in the File Server, at most 65,535 files will be listed.
   * @throws IOException If an error occurs while communicating with the system.
   * 
   ...
   *         empty object array is returned. <br>
   *         Note: Due to a limitation in the File Server, at most 65,535 files will be listed.
   * @throws IOException If an error occurs while communicating with the system.
   * 
   * @see #setPatternMatching(int)
   ...
   * system.
   * <p>
   * When <code>pattern</code> is "*" and the IFSFile object represents the root of the QSYS file system, this method
   ...
   *         attribute information will not be valid. <br>
   *         Note: Due to a limitation in the File Server, at most 65,535 files will be listed.
   * @throws IOException If an error occurs while communicating with the system.
   * 
   * @see #setPatternMatching(int)
   ...
   *         empty directory, or the pattern does not match any files, an empty object array is returned. <br>
   *         Note: Due to a limitation in the File Server, at most 65,535 files will be listed.
   * @throws IOException If an error occurs while communicating with the system.
   * 
   * @see #setPatternMatching(int)
   ...
   * 
   * @param accessType The type of access to set on the file. Valid values are: <ul> <li> {@link #ACCESS_READ
   * ACCESS_READ} <li> {@link #ACCESS_WRITE ACCESS_WRITE} <li> {@link #ACCESS_EXECUTE ACCESS_EXECUTE} </ul>
   * 
   * @param accessMode If true, sets the access permission to allow the specified type of operation; if false, to
   * disallow the operation.
   * 
   * @param ownerOnly If true, the specified permission applies only to the owner's permission; otherwise, it applies to
   * everybody.
   * 
   * @return true if successful; false otherwise.
   * 
  ...
  boolean setAccess(int accessType, boolean enableAccess, boolean ownerOnly) throws IOException, AS400SecurityException
  {
      ...
      if (accessType != ACCESS_READ && accessType != ACCESS_WRITE && accessType != ACCESS_EXECUTE)
          throw new ExtendedIllegalArgumentException("accessType ("+accessType+")", ExtendedIllegalArgumentException.PARAMETER_VALUE_NOT_VALID);
   ...
   * 
   * @param ccsid The file data CCSID. Note that the data in the file is not changed; only the CCSID "tag" on the file
   *              is changed.
   * @return true if successful; false otherwise.
   * @throws IOException If an error occurs while communicating with the system.
   * 
   * @exception ConnectionDroppedException If the connection is dropped unexpectedly.
   * @exception ExtendedIOException        If an error occurs while communicating with the system, or if the file
   ...
   * 
   * @param attributes The attributes to set on the file. These attributes are <i>not</i> ORed with existing attributes.
   * They replace the existing fixed attributes of the file. The attributes are a bit map as follows <UL> <li>0x0001: on
   * = file is a readonly file <li>0x0002: on = file is a hidden file <li>0x0004: on = file is a system file <li>0x0010:
   * on = file is a directory <li>0x0020: on = file has been changed (archive bit) </UL> For example, 0x0023 is a
   * readonly, hidden file with the archive bit on.
   * 
   * @return true if successful; false otherwise.
   * 
   * @exception ConnectionDroppedException If the connection is dropped unexpectedly.
  ...
  boolean setFixedAttributes(int attributes) throws IOException
  {
      ...
      if ((attributes & 0xFFFFFF00) != 0)
          throw new ExtendedIllegalArgumentException("attributes ("+attributes+")", ExtendedIllegalArgumentException.PARAMETER_VALUE_NOT_VALID);
   ...
   * 
   * @param attribute True to set the hidden attribute of the file. False to turn off the hidden attribute.
   * 
   * @return true if successful; false otherwise.
   * @throws IOException If an error occurs while communicating with the system.
   * 
   ...
   * 
   * @param time The desired last modification time (measured in milliseconds since January 1, 1970 00:00:00 GMT), or 0
   *             to leave the last modification time unchanged, or -1 to set the last modification time to the current
   *             system time.
   * 
   * @return true if successful; false otherwise.
   * @throws IOException If an error occurs while communicating with the system.
   ...
   * 
   * @param time The desired last modification time (measured in milliseconds since January 1, 1970 00:00:00 GMT), or 0
   * to leave the last modification time unchanged, or -1 to set the last modification time to the current system time.
   * 
   * @return true if successful; false otherwise.
   * 
   * @exception ConnectionDroppedException If the connection is dropped unexpectedly.
  ...
  boolean setLastModified0(long time) throws IOException, AS400SecurityException
  {
      ...
      if (time < -1)
      {
          throw new ExtendedIllegalArgumentException("time (" + Long.toString(time) + ")",
  ...
  boolean setLength0(int length) throws IOException, AS400SecurityException
  {
      ...
      if (length < 0)
      {
          throw new ExtendedIllegalArgumentException("length (" + Integer.toString(length) + ")",
                                                     ExtendedIllegalArgumentException.PARAMETER_VALUE_NOT_VALID);
      }
      
      // Note: The file system will not allow us to set the length of a file to a value larger than (2Gig minus 1),
      // or 2147483647 (0x7FFFFFFF) bytes, which happens to be the maximum positive value which an 'int' will hold. 
      // Therefore we do not provide a setLength(long) method.
  ...
  /**
   * Sets the file path. An exception will be returned if the IFSFile object 
   * has been operated on that results in a server connection being established.
   * 
  ...
  public void setPath(String path) throws PropertyVetoException
  {
      ...
      // Ensure that the path is not altered after the connection is established.
      if (impl_ != null)
      ...
      String newPath = normalizePath(null, path);
   ...
   * 
   * @param patternMatching Either {@link #PATTERN_POSIX PATTERN_POSIX}, {@link #PATTERN_POSIX_ALL PATTERN_POSIX_ALL},
   *                        or {@link #PATTERN_OS2 PATTERN_OS2}
   * @throws IOException If an error occurs while communicating with the system.
   * 
           ...
           UnknownHostException
  {
      ...
      if (system == null)
          throw new ExtendedIllegalArgumentException("permission.system (null)", ExtendedIllegalArgumentException.PARAMETER_VALUE_NOT_VALID);
      ...
      if (system.equals(system_))
      {
          ...
          {
              throw new ExtendedIllegalArgumentException("permission.objectPath (" + permission.getObjectPath() + ")",
                                                         ExtendedIllegalArgumentException.PARAMETER_VALUE_NOT_VALID);
          }
      }
      else
      {
          ...
          throw new ExtendedIllegalArgumentException("permission.system (" +  systemName + ")",
                                                     ExtendedIllegalArgumentException.PARAMETER_VALUE_NOT_VALID);
      }
  }
   ...
   * 
   * @param attribute True to set the read only attribute of the file such that the file cannot be changed. False to set
   *                  the read only attributes such that the file can be changed.
   * 
   * @return true if successful; false otherwise.
   * @throws IOException If an error occurs while communicating with the system.
   * 
  ...
  /**
   * Sets the sorting behavior used when files are listed by any of the <tt>list()</tt> or <tt>listFiles()</tt> methods.
   * The default is <tt>false</tt> (unsorted).
   * 
   * @param sort If <tt>true</tt>: Lists of files are returned in sorted order. If <tt>false</tt>: Lists of files are
   *             returned in whatever order the file system provides.
   * 
   * @exception IOException            If an error occurs while communicating with the system.
   * @exception AS400SecurityException If a security or authority error occurs.
   **/
   ...
   * INTERNAL USE ONLY - SUBJECT TO CHANGE
   * 
   * This function normalizes the path of the object in IFSFile. If a directory path 
   * is passed, it will be prepended to the path (or name).  It will also ensure
   * that the resultant path is absolute (i.e. starts from the root ('/')). 
   * 
   * Note: The path parameter can be a simple name or path. 
   */
  static String normalizePath(String dir, String path)
  {
      StringBuilder sb = new StringBuilder();
      
      // Include directory in path if there is one.
      if (dir != null)
      {
          sb.append(dir);
          if (path != null && sb.length() != 0 && sb.charAt(sb.length() - 1) != separatorChar)
              sb.append(separatorChar);
      }
      
      // If path/name specified, append it. 
      if (path != null)
          sb.append(path);

      // If the specified path doesn't start with the separator character,
      // add one.  All paths are absolute for IFS.
      if (sb.length() == 0 || sb.charAt(0) != separatorChar)
