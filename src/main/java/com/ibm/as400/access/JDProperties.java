...
public class JDProperties implements Serializable, Cloneable {
  static final String copyright = "Copyright (C) 1997-2025 International Business Machines Corporation and others.";
  ...
  // following indices.
  //
  // New properties must be added to the end of the list!!!
  //
  ...
  // nstatic final String PROXY_SERVER_SECURE_ = "proxy server secure";
  ...
  static final String EMPTY_ = ""; // @pw1 provide way to know if property was set here or by application via (Str
                                   // == JDProperties.EMPTY)
  ...
  // @F4D static final String DRIVER_DEFAULT = "default";
  ...
  static final String BIDI_STRING_TYPE_NOTSET = EMPTY_;
  ...
  static final String TRACE_TOOLBOX_NONE = NONE_;
  ...
  static final String METADATA_SOURCE_HOST_VERSION_DEFAULT = "-1"; // @mdsp //not yet set, so default depends on i5
                                                                   // version (post-v6r1 default is SP)
  ...
  static {
    ...
    // been defined:
    // 1 - (v5r2f) Don't throw exception of executeQuery() does not return a result
    // set
    // Native Driver has reserved the following bits:
    // 0x02 to turn on these additional warnings in V5R2 (DATA TRUNCATION and DATA
    // MAPPING)
    ...
    dpi_[i].description = "EXTENDED_METADATA_DESC"; 
    dpi_[i].required = false;
    dpi_[i].choices = new String[2];
    dpi_[i].choices[0] = FALSE_;
    dpi_[i].choices[1] = TRUE_;
    ...
    dpi_[i].description = "LOGIN_TIMEOUT_DESC"; 
    dpi_[i].required = false;
    dpi_[i].choices = new String[0];
    defaults_[i] = EMPTY_;
    ...
    dpi_[i].choices[1] = SORT_LANGUAGE1; 
    dpi_[i].choices[2] = SORT_TABLE1; 
    defaults_[i] = SORT_HEX; 

    // Sort language.
    //
    // We set the default to Engligh United States. At first, we set 
    // it to " " (3 spaces), but that causes a host server error. 
    // It would probably be better to choose a default based on the client 
    // locale, but that may prove to be a high-maintenance mapping, 
    // as locales are added to Java and languages are added to the system. 
    i = SORT_LANGUAGE;
    dpi_[i] = new DriverPropertyInfo(SORT_LANGUAGE_, "");
    dpi_[i].description = "SORT_LANGUAGE_DESC";
    dpi_[i].required = false;
    dpi_[i].choices = new String[0];
    defaults_[i] = SORT_LANGUAGE_ENGLISH_UNITED_STATES;
    ...
    for (int j = 0; j < 64; ++j)
      dpi_[i].choices[j] = Integer.toString(j);
    ...
    dpi_[i].description = "USE_SOCK5_DESC";
    dpi_[i].required = false;
    dpi_[i].choices = new String[2];
    dpi_[i].choices[0] = FALSE_;
    dpi_[i].choices[1] = TRUE_;
  ...
  JDProperties(Properties urlProperties, Properties info, char[] password, char[] additionalAuthenticationFactor) {
    ...
    for (int i = 0; i < NUMBER_OF_ATTRIBUTES_; ++i) {
      if (i != PASSWORD) { /* @AI7A */
        ...
        String propertyValue = getProperty(urlProperties, info, supportedProperty);
  ...
  // @PDA 550 - clone
  /**
   * Method to create a clone of JDProperties. This makes a semi-deep copy of
   * values_ array, so both values_ array references point to separate array
   * objects.
   */
  public Object clone() {
    try {
      ...
      // since Strings are immutable, no need to clone String objects that both arrays
      // refer to.
      return clone;
  ...
  /**
   * Is the value of the specified property set to the specified value? The
  ...
  /**
   * Get the value of the specified property as a boolean. This is intended for
   * properties that take "true" and "false" as values.
   * 
   * @param index Property index.
   * @return The value.
   **/
  ...
  /**
   * Get the index of the value of the specified property. This is the index
   * within the list of choices. If the property is not specified, then the index
   * of the default value will be returned. If the value does not match a choice,
   * then return -1.
   * 
   * @param index Property index.
  ...
  /**
   * Get the value of the specified property as an int. This is intended for
   * properties that take integers as values.
   * 
   * @param index Property index.
   * @return The value.
   **/
    ...
    try {
      ...
      if (JDTrace.isTraceOn())
        JDTrace.logInformation(this,
            "NumberFormatException processing propertyIndex[" + index + "]='" + values_[index] + "'");
  ...
  /**
   * Get the original "info" Properties object, that was passed in as the second
  ...
  // @PDA
  /**
   * Gets index of the property name as indexed in array. This method is used by
   * AS400JDBCDataSource so that it can update properties based on property name
   * string.
   * 
   * @param propertyName
  ...
  /**
   * Returns the value of the specified property. The URL properties are searched
   * first, then the info properties.
   * 
   * @param urlProperties The URL properties.
   * @param info          The info properties.
  ...
  /**
   * Get the value of the specified property. If the property is not specified,
   * then the default value will be returned. If choices are allowed, then the
   * value will be compared against the choices, and if none match, the default
  ...
  /**
   * Gets a custom SSL Socket Factory, or returns <tt>null</tt> if no custom SSL
   * Socket factory is specified (in which case, the system default factory will
   * be used).
   * 
  ...
  /**
   * Indicates if the trace property is set. This needs to be detected before an
   * object of this class is even instantiated, which is why it is static.
   * 
   * @param urlProperties The URL properties.
   * @param info          The info properties.
   * @return Whether the trace property was set to true, false, or not specified
   *         when constructed.
   **/
  static String isTraceSet(Properties urlProperties, Properties info) // @E7C
  {
    ...
    // @E7D return Boolean.valueOf (getProperty (urlProperties, info,
    // TRACE_)).booleanValue();
  }
  ...
  //
  // This scheme works ONLY IF NEW ELEMENTS ARE ALWAYS ADDED TO THE END OF THE
  // ARRAY!!!!
  //
  // @param in The input stream from which to deserialize the object.
  // @exception ClassNotFoundException If the class being deserialized is not
  // found.
  ...
  /**
   * Set the value of the specified property. If choices are allowed, then the
   * value will be compared against the choices, and if none match, the property
  ...
  void setString(int index, String value) {
    ...
    if ((index != USER) && (index != PASSWORD) && ((value == null) || ((value.length() == 0))))
    ...
    if ((index == PACKAGE_CCSID) || (index == PORTNUMBER) || (index == MAX_RETRIES_FOR_CLIENT_REROUTE)
        || (index == RETRY_INTERVAL_FOR_CLIENT_REROUTE) || (index == AFFINITY_FAILBACK_INTERVAL)) {
      try {
    ...
    if (JDTrace.isTraceOn()) {
      JDTrace.logProperty(this, "setString", dpi_[index].name,
