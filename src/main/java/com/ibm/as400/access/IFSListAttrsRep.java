...
class IFSListAttrsRep extends IFSDataStream
{
    ...
    private final byte[] getObjAttrBytes(int type)
    {
        ...
        while (buf == null && offset < data_.length)
        {
            ...
            { 
                // We found an OA* of the desired type.
            ...
            { 
                // not what we're looking for, so keep looking
    ...
    {
        // Assume that this reply has an "Optional/Variable" section, and that it contains an "Object Attribute 1"
        // structure.
    ...
    long getSize(int datastreamLevel)
    {
        ...
        if (datastreamLevel < 8 || datastreamLevel == 0xF4F4)
        {
            ...
            int size = get32bit(FILE_SIZE_OFFSET);
     ...
     * 
     * @return true if object is a symbolic link; false if not a symbolic link (or if could not determine).
     **/
    ...
    int getFileSystemType(int datastreamLevel)
    {
        ...
        if (datastreamLevel < 8 || datastreamLevel == 0xF4F4)
            Trace.log(Trace.WARNING, "Could not determine the file system type, field is returned for data stream levels 8 and later.");
        else
