...
class IFSListAttrsReq extends IFSDataStreamReq
{
     ...
     * @param authority         bit 0: on = client must have read authority, bit 1: on = client must have write
     *                          authority, bit 2: on = client must have execute authority
     ...
     * @param isRestartName     true: interpret restartNameOrID as a restart Name, false: interpret restartNameOrID as a
     *                          restart ID.
     * @param eaNames           The list of extended attribute names, or null to return no extended attributes.
     * @param eaNameBytesLength The total number of bytes in the eaNames list.
     * @param longFileSize      true: return file size as 8-byte value (as optional field), false: do not return 8-byte
     *                          file-size field.
     **/
    IFSListAttrsReq(byte[] name, int fileNameCCSID, int authority, int maximumGetCount, byte[] restartNameOrID,
            boolean isRestartName, byte[][] eaNames, int eaNameBytesLength, boolean longFileSize, int patternMatching)
    {
        ...
        super(HEADER_LENGTH + TEMPLATE_LENGTH + LLCP_LENGTH + name.length
                + ((restartNameOrID != null) ? (LLCP_LENGTH + restartNameOrID.length) : 0) 
                + ((eaNames == null) ? 0 : (8 + 10*eaNames.length + eaNameBytesLength)));
        ...
        if (maximumGetCount <= 0)
            ...
            set16bit(maximumGetCount, MAX_GET_COUNT_OFFSET);                     

        // Set the file attribute list level.
        if (longFileSize == true)
        ...
        int offset = FILE_NAME_OFFSET + name.length; // Offset for next field.

        // Set the "restart name", if specified.
        if (restartNameOrID != null)
        {
            int codePoint;
            if (isRestartName)
                codePoint = 0x0007; // it's a Restart Name
            else
                codePoint = 0x000E; // it's a Restart ID
            
            set32bit(restartNameOrID.length + LLCP_LENGTH, offset); // LL
            set16bit(codePoint, offset + 4); // CP
            System.arraycopy(restartNameOrID, 0, data_, offset + LLCP_LENGTH, restartNameOrID.length);
            
            // It would would be more efficient to use the ordinal values
            // (which work outside of QSYS and QDLS). I chose to keep it
            // simple for now, but we should make it more sophisticated later.
            ...
            offset += LLCP_LENGTH + restartNameOrID.length; // next field
        }

        // Set the "extended attribute names", if specified.
        if (eaNames != null)
        {
            // Set EA List Length:
            ...
            for (int i = 0; i < eaNames.length; i++)
            {
                ...
                System.arraycopy(eaNames[i], 0, data_, offset + 10, eaNames[i].length);
  ...
  {
      super(HEADER_LENGTH + TEMPLATE_LENGTH + (attrsType == OA1 ? 14 : 0) + (attrsType == OA12 ? 14 : 0));
      setLength(data_.length);
      setTemplateLen(TEMPLATE_LENGTH);
      setReqRepID(0x000A);
      ...
      case OA12:
          ...
          break;
      default:
          // do not use a file handle, and do not return an OA* structure
  ...
  /**
   * Sets the value of the "pattern matching" field. Valid values are "POSIX" (0), "POSIX-all" (1), and "OS/2" (2).
   **/
  final void setPatternMatching(int patternMatching)
  {
      // "POSIX" pattern matching: Using POSIX semantics, return all files that match the pattern and do not begin with
      // a period unless the pattern begins with a period. In that case, names beginning with a period will be returned.

      // "POSIX-all" pattern matching: Using POSIX semantics, return all files that match the pattern, including those
   ...
   * 
   * @param sort If true: Return the directory information in sorted order. If false: Return the directory information
   *             in the order the file system provides.
   **/
  final void setSorted(boolean sort)
  {
      ...
      if (sort)
          attributeListLevel = attributeListLevel | 0x0080; // Turn on bit 7.
      else
          attributeListLevel = attributeListLevel & 0xFF7F; // Turn off bit 7.
