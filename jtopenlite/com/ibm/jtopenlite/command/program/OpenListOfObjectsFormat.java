 ...
 *
 */
public class OpenListOfObjectsFormat extends ListEntryFormatAdapter
{
  private OpenListOfObjectsFormatListener listener_;
  ...
  synchronized public OpenListOfObjectsFormatListener getListener()
  {
    return listener_;
  }

  synchronized public void setListener(OpenListOfObjectsFormatListener listener)
  {
    listener_ = listener;
  }
...
//      String objectType = Conv.ebcdicByteArrayToString(data, numRead, 10, charBuffer_);
      ...
      numRead += 4;
      listener_.newObjectEntry(objectName, objectLibrary, objectType, informationStatus, numFields);
      if (!informationStatus.equals("A") && !informationStatus.equals("L"))
      {
        for (int i=0; i<numFields; ++i)
        {
          ...
          listener_.newObjectFieldData(lengthOfFieldInfo, keyField, typeOfData, lengthOfData, numRead+16, data);
