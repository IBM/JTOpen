...
class ListDiskStatusesImpl implements OpenListOfSpooledFilesFormatOSPL0300Listener, OpenListOfSpooledFilesFilterOSPF0100Listener, DDMReadCallback
{
  ...
  public void newRecord(DDMCallbackEvent event, DDMDataBuffer dataBuffer)
  {
    ...
    else if (!theEnd_)
    {
      ...
      else
      {
        ...
        String sizeMB = st.nextToken();
        String percentUsed = st.nextToken();
        String ioRequests = st.nextToken();
        String requestSizeKB = st.nextToken();
        String readRequests = st.nextToken();
        String writeRequests = st.nextToken();
        String readKB = st.nextToken();
        String writeKB = st.nextToken();
        String percentBusy = st.nextToken();
        String asp = st.nextToken();
        String protectionType = st.nextToken();
        String protectionStatus = st.nextToken();
        String compression = st.hasMoreTokens() ? st.nextToken() : null; //TODO
        DiskStatus ds = new DiskStatus(unit, type, sizeMB, percentUsed, ioRequests, requestSizeKB, readRequests,
                                       writeRequests, readKB, writeKB, percentBusy, asp, protectionType, protectionStatus, compression);
  ...
  **/
  public DiskStatus[] getDiskStatuses(final CommandConnection cc, final DDMConnection ddmConn, String workingLibrary) throws IOException
  {
...
//    DDMConnection conn = DDMConnection.getConnection("rchasa12", "csmith", "s1r4l0in");
//    CommandConnection conn = CommandConnection.getConnection("rchasa12", "csmith", "s1r4l0in");
//    System.out.println(conn.getJobName());
    ...
    if (!result.succeeded()) throw new IOException("Error creating library: "+result.toString());
    result = cc.execute("CRTPF "+workingLibrary+"/DSKSTS RCDLEN(132) MAXMBRS(*NOMAX) SIZE(*NOMAX) LVLCHK(*NO)");
    if (!result.succeeded()) throw new IOException("Error creating physical file: "+result.toString());
    result = cc.execute("CRTOUTQ OUTQ("+workingLibrary+"/DSKSTS)");
    if (!result.succeeded()) throw new IOException("Error creating output queue: "+result.toString());
    result = cc.execute("CHGJOB OUTQ("+workingLibrary+"/DSKSTS)");
    if (!result.succeeded()) throw new IOException("Error changing job: "+result.toString());
    result = cc.execute("WRKDSKSTS OUTPUT(*PRINT)");
    if (!result.succeeded()) throw new IOException("Error running WRKDSKSTS: "+result.toString());
    OpenListOfSpooledFilesFormatOSPL0300 format = new OpenListOfSpooledFilesFormatOSPL0300(this);
    OpenListOfSpooledFiles list = new OpenListOfSpooledFiles(format, 256, -1, null, this, null, null, null);
    result = cc.call(list);
    if (!result.succeeded()) throw new IOException("Error retrieving spooled file: "+result.toString());
    ListInformation info = list.getListInformation();
    CloseList close = new CloseList(info.getRequestHandle());
    result = cc.call(close);
    if (!result.succeeded()) throw new IOException("Error closing spooled file list: "+result.toString());
    String jobID = jobNumber_.trim()+"/"+jobUser_.trim()+"/"+jobName_.trim();
    ...
    if (!result.succeeded()) throw new IOException("Error copying spooled file: "+result.toString());
...
/*
    ...
    java.sql.Connection c = DriverManager.getConnection("jdbc:systemi://rchasa12", "csmith", "s1r4l0in");
    Statement s = c.createStatement();
    ResultSet rs = s.executeQuery("SELECT * FROM QZRDDSKSTS.DSKSTS");
    int skip = 0;
    ...
    while (rs.next())
    {
      ...
      else if (!theEnd)
      {
        ...
        else
        {
          ...
          String protectionType = st.nextToken();
          String protectionStatus = st.nextToken();
          String compression = st.nextToken(); //TODO
        }
      }
    }
    rs.close();
