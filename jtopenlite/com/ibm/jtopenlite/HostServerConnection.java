 ...
 * All HostServerConnections have associated system information provided via the SystemInfo class.
**/
public abstract class HostServerConnection implements Connection
{
  ...
  protected HostServerConnection(SystemInfo info, String user, String jobName, Socket socket, HostInputStream in, HostOutputStream out)
  {
    info_ = info;
    user_ = user;
    jobName_ = jobName;
  ...
  **/
  ...
  /**
   * If <i>debug</i> is true, enables datastream debugging for this connection.
  **/
  ...
  **/
  ...
  protected static byte[] getEncryptedPassword(byte[] userBytes, byte[] passwordBytes, byte[] clientSeed, byte[] serverSeed, int passwordLevel) throws IOException
  {
  ...
  protected static String connect(SystemInfo info, HostOutputStream dout, HostInputStream din, int serverID, String user, String password) throws IOException
  {
    ...
    if (rc != 0)
    {
      ...
      throw msg == null ? DataStreamException.badReturnCode("startServer-"+serverID, rc) :
                          DataStreamException.errorMessage("startServer-"+serverID, new Message(String.valueOf(rc), msg));
    }
    String jobName = null;
  ...
  {
    if ((rc & 0xFFFF0000) == 0x00010000) return "Error on request data";
    if ((rc & 0xFFFF0000) == 0x00040000) return "General security error, function not performed";
    if ((rc & 0xFFFF0000) == 0x00060000) return "Authentication Token error";
    switch (rc)
    {
      case 0x00020001: return "Userid error: User Id unknown";
      case 0x00020002: return "Userid error: User Id valid, but revoked";
      case 0x00020003: return "Userid error: User Id mismatch with authentication token";
      case 0x0003000B: return "Password error: Password or Passphrase incorrect";
      case 0x0003000C: return "Password error: User profile will be revoked on next invalid password or passphrase";
      case 0x0003000D: return "Password error: Password or Passphrase correct, but expired";
      case 0x0003000E: return "Password error: Pre-V2R2 encrypted password";
      case 0x00030010: return "Password error: Password is *NONE";
    }
    return null;
  }

  static void sendStartServerRequest(HostOutputStream out, byte[] userBytes, byte[] encryptedPassword, int serverID) throws IOException
  {
  ...
  static long sendExchangeRandomSeedsRequest(HostOutputStream out, int serverID) throws IOException
  {
  ...
  protected static final class HostInputStream
  {
    ...
    private void debugByte(int i)
    {
      ...
      if (debugCounter_ == 0)
      {
        synchronized (HostOutputStream.formatter_)
        {
          ...
          tracePrintStream.print(HostOutputStream.formatter_.format(new java.util.Date()));
      ...
      if (++debugCounter_ % 16 == 0) tracePrintStream.println();
      else tracePrintStream.print(" ");
      }
    }
    ...
    **/
    ...
    public int readShort() throws IOException
    {
      ...
      return((shortArray_[0] & 0x00FF) << 8) |
      (shortArray_[1] & 0x00FF);
    }
  ...
  protected static final class HostOutputStream
  {
    ...
    private void debugByte(int i)
    {
      ...
      if (debugCounter_ == 0)
      {
        ...
        {
          tracePrintStream.print(formatter_.format(new java.util.Date()));
  ...
  protected static void writePadEBCDIC10(String s, HostOutputStream out) throws IOException
  {
    Conv.writePadEBCDIC10(s, out);
  }

  protected static void writePadEBCDIC(String s, int len, HostOutputStream out) throws IOException
  {
    Conv.writePadEBCDIC(s, len, out);
  }

  protected static void writeStringToUnicodeBytes(String s, HostOutputStream out) throws IOException
  {
